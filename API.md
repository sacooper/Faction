...
```javascript
{
    ...
    factionsReceived: [], // (*) array of {sender, story, fact, factionId, createdAt, comments, imageUrl (optional)}
    factionsSent: [], // (*) an array of {recipients, story, fact, factionId, createdAt, comments, imageUrl (optional) }
    pendingFactions: [], // (*) an array of {sender, story, fact, factionId, createdAt, comments, imageUrl (optional)}
    /* 
...
```javascript
{
    receivedFriendRequests: [], // (*) username strings (they are awaiting an answer from you)
    acceptedFriendRequests: [], // (1) username strings (they are your new friends)
    pendingFactions: [],        // (*) array of {sender, story, fact, factionId, comments, imageUrl (optional)}
    /* 
        sender is a username string
        story is a string
        fact is a boolean
